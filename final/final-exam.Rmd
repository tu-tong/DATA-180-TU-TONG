...
f) With this data, I would fit a supervised learning model. Because there is one response variable to all the predictor variables in the data set.
...
- Convert each category to a specific number. For example, 'highschool' to 0, 'college' to 1, and 'graduate' to 2.
- Convert by using the matrix of 0 and 1 (binary). Count the existence of each category, so each category is now displayed as a row. For instance, the value of that row will be 1 if the categorical value matches with the row.
