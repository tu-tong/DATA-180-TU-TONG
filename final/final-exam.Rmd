...
e) We should deal with missing values by removing the rows or columns that contain the missing values or substitute missing values with other values (such as sample mean). However, there is no best way to approach.

f) With this data, I would fit a supervised learning model. Because there is one response variable to all the predictor in the data set 
...
- Covert each category to a specific number. For example, 'highschool' to 0, 'college' to 1, and 'graduate' to 2.
- Count the existence of each category, so each category is now displayed as a row. For instance, the value of that row will be 1 if the categorical value matches with the row.

## Data Visualization

a) Bar plot for 'Def_ind'
The graph has 2 values 0 and 1 on the x-axis, the y-axis shows the frequency of each x-value appear in the data set.
```{r}
ggplot(new_loan_df, aes(x=Def_ind))+
...
fivenum(new_loan_df$tot_balance)
```
